...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MHAcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MHAcoin Signed Message:\n";
...
{
    // MHAcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // MHAcoin: 20,000

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // MHAcoin: 1 day
static const int64 nTargetSpacing = 120; // MHAcoin: 2 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MHAcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 120; // MHAcoin: 2 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // MHAcoin: 1 day
...
{
    // MHAcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "Mental Health Association";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // MHAcoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MHAcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MHAcoinMiner : ProcessBlock, block not accepted");
    }
